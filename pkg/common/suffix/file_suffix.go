...
func init() {
	...
	fileTypeMap.Store("424d228c010000000000", "bmp")  	// 16-color bitmap (bmp)
	fileTypeMap.Store("424d8240090000000000", "bmp")  	// 24-bit bitmap (bmp)
	fileTypeMap.Store("424d8e1b030000000000", "bmp")  	// 256-color bitmap (bmp)
	...
	fileTypeMap.Store("d0cf11e0a1b11ae10000", "vsd")  	// Visio Drawing
	...
	fileTypeMap.Store("d0cf11e0a1b11ae10000", "doc")          	// MS Excel Note: The file headers of word, msi and excel are the same
	fileTypeMap.Store("504b0304140006000800", "docx")         	// docx file
	fileTypeMap.Store("d0cf11e0a1b11ae10000", "wps")          	// WPS text wps, table et, demo dps are all the same

	fileTypeMap.Store("2e524d46000000120001", "rmvb")	// rmvb or rm
	fileTypeMap.Store("464c5601050000000900", "flv")  	// flv or f4v
	...
	fileTypeMap.Store("3026b2758e66cf11a6d9", "wmv") 	// wmv or asf
	...
	fileTypeMap.Store("4d5a9000030000000400", "exe")        	// exe file
	fileTypeMap.Store("3c25402070616765206c", "jsp")        	// jsp file
	fileTypeMap.Store("4d616e69666573742d56", "mf")         	// MF file
	fileTypeMap.Store("3c3f786d6c2076657273", "xml")        	// xml file
	fileTypeMap.Store("494e5345525420494e54", "sql")        	// xml file
	fileTypeMap.Store("7061636b616765207765", "java")       	// java file
	fileTypeMap.Store("406563686f206f66660d", "bat")        	// bat file
	fileTypeMap.Store("1f8b0800000000000000", "gz")         	// gz file
	fileTypeMap.Store("6c6f67346a2e726f6f74", "properties") 	// bat file
	fileTypeMap.Store("cafebabe0000002e0041", "class")      	// bat file
	fileTypeMap.Store("49545346030000006000", "chm")        	// bat file
	fileTypeMap.Store("04000000010000001300", "mxp")        	// bat file
...
// get the binary of the previous result bytes
...
// use the first few bytes of the file to judge
// fSrc: file byte stream (just use the first few bytes)
